...
class CreateUserSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if not re.search('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`\'…》]', attrs['password']) or not re.search('[A-Z]', attrs['password']) or not re.search('[a-z]', attrs['password']) or not re.search('[0-9]', attrs['password']):
            raise serializers.ValidationError("패스워드는 영어 대문자, 영어 소문자, 특수 문자, 숫자 각 하나 이상 포함되어야 합니다.")
